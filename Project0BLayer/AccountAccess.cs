...
namespace Project0Blayer
{
    public class AccountAccess 
    {
        ...
        public int WelcomeMessage()
        {
            ...
            int input = 0;
            bool choice = Int32.TryParse(Console.ReadLine(), out input);
            //if (input != 1 || input != 2)
            //{
        ...
        public int HasAccessExistingAccount( int input, out string userName)
        {
           ...
           var accounts = _context.Customers.Where(x => x.AccountUserName == inputUN).ToList();
            foreach (var person in accounts)
            {
                if(inputUN.Trim().ToLower() != person.AccountUserName.Trim().ToLower())
                {
                ...
                {
                    userName = person.AccountUserName;
                    return person.AcStore;
                }
            }
